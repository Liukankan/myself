...
function doMove ( obj, attr, step, target, time,callback ) {
...
function timeCountDown(oSetTime){
...
function setCookie(key,value,time,codeFun){
		var oDate = new Date();
		oDate.setDate(oDate.getDate()+time);
		oDate.toGMTString();
		document.cookie = key+'='+codeFun(value)+';expires='+oDate;
}
function getCookie(key,decodeFun){
...
function setOpacity(obj,speed,target,time,endfun){
...
function setAttr(obj,attr,speed,target,time,endfun){
		var iCur =0;
...
function showDown(obj,json,time,endfun){
		clearInterval(obj.timer);
		obj.timer = setInterval(function(){
		var iStop = true;
...
function moveFlex(obj,json,time,endfun){
